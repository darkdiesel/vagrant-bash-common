...
if [ -f $VAGRANT_APT_SOURCE ]; then
    ...
    sudo echo "deb [signed-by=/usr/share/keyrings/mariadb-keyring.pgp] http://mirror.aarnet.edu.au/pub/MariaDB/repo/11.4/ubuntu mantic main" >> $MARIADB_APT_SOURCE_LIST
    sudo echo "deb-src [signed-by=/usr/share/keyrings/mariadb-keyring.pgp] http://mirror.aarnet.edu.au/pub/MariaDB/repo/11.4/ubuntu mantic main" >> $MARIADB_APT_SOURCE_LIST
