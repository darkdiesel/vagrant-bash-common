...
if [ $(dpkg-query -W -f='${Status}' php7.3-fpm 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
...
sudo cp -R ${VAGRANT__DEBIAN_COMMON_CONFIGS_PATH}/etc/php/7.3/* /etc/php/7.3/ > /dev/null 2>&1
