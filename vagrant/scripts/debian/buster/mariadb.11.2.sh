...
VAGRANT_APT_SOURCE=${VAGRANT__OS_CONFIGS_PATH}"/etc/apt/sources.list.d/mariadb.11.2.list"
...
if [ -f $VAGRANT_APT_SOURCE ]; then
    ...
    sudo echo "deb [arch=amd64,i386,ppc64el] http://mirror.hosting90.cz/mariadb/repo/11.2/debian buster main" >> $MARIADB_APT_SOURCE_LIST
    sudo echo "deb-src http://mirror.hosting90.cz/mariadb/repo/11.2/debian buster main" >> $MARIADB_APT_SOURCE_LIST
