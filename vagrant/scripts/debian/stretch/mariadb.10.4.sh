...
VAGRANT_APT_SOURCE=${VAGRANT__OS_CONFIGS_PATH}"/etc/apt/sources.list.d/mariadb.10.4.list"
...
if [ -f $VAGRANT_APT_SOURCE ]; then
    ...
    sudo echo "deb [arch=amd64,i386,ppc64el] http://mirror.hosting90.cz/mariadb/repo/10.4/debian stretch main" >> $MARIADB_APT_SOURCE_LIST
    sudo echo "deb-src http://mirror.hosting90.cz/mariadb/repo/10.4/debian stretch main" >> $MARIADB_APT_SOURCE_LIST
