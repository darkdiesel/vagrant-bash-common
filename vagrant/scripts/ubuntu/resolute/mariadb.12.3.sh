...
VAGRANT_APT_SOURCE=${VAGRANT__OS_CONFIGS_PATH}"/etc/apt/sources.list.d/mariadb.12.3.list"
...
if [ -f $VAGRANT_APT_SOURCE ]; then
    ...
    sudo echo "deb [signed-by=/etc/apt/keyrings/mariadb-keyring.asc] https://mariadb.mirror.serveriai.lt/repo/12.3/ubuntu resolute main" >> $MARIADB_APT_SOURCE_LIST
    sudo echo "deb-src [signed-by=/etc/apt/keyrings/mariadb-keyring.asc] https://mariadb.mirror.serveriai.lt/repo/12.3/ubuntu resolute main" >> $MARIADB_APT_SOURCE_LIST
