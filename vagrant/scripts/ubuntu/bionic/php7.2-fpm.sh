...
if [ $(dpkg-query -W -f='${Status}' php-fpm 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
    ...
    sudo apt-get install -y php7.2-fpm php7.2-mbstring php7.2-gd php7.2-curl php7.2-intl php7.2-xml php7.2-mysql php7.2-zip php-xdebug > /dev/null 2>&1
...
sudo cp -R ${VAGRANT_OS_CONFIGS_DIR}/etc/php/7.2/* /etc/php/7.2/ > /dev/null 2>&1
